...
INLINE BBox3 operator*(const Matrix3 &tm, const BBox3 &b)
{
  ...
  for (int i = 0; i < 2; ++i)
    for (int j = 0; j < 2; ++j)
    {
      // z = b.lim[0].z
      {
        const Point3 p(b.lim[i].x, b.lim[j].y, b.lim[0].z);
        Point3 tp;
        tp.x = tm.m[0][0] * p.x + tm.m[1][0] * p.y + tm.m[2][0] * p.z;
        tp.y = tm.m[0][1] * p.x + tm.m[1][1] * p.y + tm.m[2][1] * p.z;
        tp.z = tm.m[0][2] * p.x + tm.m[1][2] * p.y + tm.m[2][2] * p.z;
        box += tp;
      }
      // z = b.lim[1].z
      {
        const Point3 p(b.lim[i].x, b.lim[j].y, b.lim[1].z);
        Point3 tp;
        tp.x = tm.m[0][0] * p.x + tm.m[1][0] * p.y + tm.m[2][0] * p.z;
        tp.y = tm.m[0][1] * p.x + tm.m[1][1] * p.y + tm.m[2][1] * p.z;
        tp.z = tm.m[0][2] * p.x + tm.m[1][2] * p.y + tm.m[2][2] * p.z;

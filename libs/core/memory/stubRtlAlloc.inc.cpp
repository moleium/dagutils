...
#include "stdRtlMemUsage.h" // For mt_*_crt, dlmalloc_usable_size, etc. declarations

// Forward declare functions potentially defined in including .cpp files
// We declare them here to satisfy linters when analyzing this file in isolation.
// In practice, this .inc.cpp file is included by other .cpp files that provide these.
extern "C" void real_init_memory();
#if MEM_DEBUGALLOC <= 0
void *mt_dlmemalign(size_t sz, size_t al);
extern "C" void memfree_anywhere(void *ptr);
void *mt_dlrealloc(void *ptr, size_t sz);
#endif

#ifdef _MSC_VER
#if _MSC_VER >= 1400 // Only include windows.h if using MSVC 2005 or later where HANDLE is likely used

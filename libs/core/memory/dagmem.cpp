...
// Note: `mallinfo` is really slow (>1ms) under some conditions, probably many
// freed/allocated small blocks, so  don't use it as we expect fast operation
// for this method. To consider: use something like RLIMIT_RSS instead?
...
#else
#pragma message(                                                               \
    "memory manager is inaccurate with _STD_RTL_MEMORY for this target")
...
#define MTR_DUMP_SIZE_PERCENTS                                                 \
  30 // workbuffer size (% of total memory needed to write all data)
...
#define MEMORY_TRACKER                                                         \
  0 // set 1 to enable memory tracker, use -sUseMemTracker=yes
#endif

#define PROTECT_MEMORY_MANAGER                                                 \
  0 // breaks execution of non-protected executables
#if DAGOR_DBGLEVEL > 0 && _TARGET_STATIC_LIB && MEM_DEBUGALLOC >= 0 &&         \
...
#define FILL_MEM_WITH_PATTERN(ptr, pattern, sz)                                \
  do {                                                                         \
    ...
    G_FAST_ASSERT(                                                             \
        blockSize <                                                            \
        (size_t)-0x10000); /*_msize returns -1 for a bad block, but            \
                              _aligned_msize subtracts the alignment*/         \
    for (int *i = (int *)(ptr), *e = i + MIN4K(blockSize / sizeof(int));       \
         i < e;)                                                               \
      *i++ = pattern;                                                          \
  } while (0)
#define FILL_PTR_WITH_PATTERN(p, pattern)                                      \
  FILL_MEM_WITH_PATTERN(p, pattern, GET_MEM_SIZE(p))
...
static volatile int cnt_expand_total_16K = 0, cnt_expand_total = 0,
                    cnt_expand_ok_16K = 0, cnt_expand_ok = 0,
                    cnt_expand_bad_p = 0;
static volatile int cnt_expand_r_total_16K = 0, cnt_expand_r_total = 0,
                    cnt_expand_r_ok_16K = 0, cnt_expand_r_ok = 0;
#endif

namespace muc2 {
#if MEMUSAGE_COUNT == 2
static constexpr int MEM_BINS_CNT = 64 /*step 8*/ + 120 /*step 64*/ +
                                    62 /*step 4K*/ + 3 /*step 256K*/ +
                                    15 /*step 1M*/ + 1 /*16M+*/;
static volatile int32_t chunkCnt[MEM_BINS_CNT] = {0},
                        chunkCntMax[MEM_BINS_CNT] = {0};
...
static void measureAlloc(void *p, ptrdiff_t sz) {
  ...
  while (cur_ptrs > max_ptrs)
    if (interlocked_compare_exchange(chunkCntMax[idx], cur_ptrs, max_ptrs) ==
        max_ptrs)
      break;
    else
...
static void measurePrintStat() {
  ...
  for (int i = 0; i < sizeof(chunkCnt) / sizeof(chunkCnt[0]); i++)
    if (chunkCnt[i] || chunkCntMax[i] >= 8) {
      ...
      debug("  %6d [%6d max]  sz[%4d%s..%4d%s),  total=%6dK", chunkCnt[i],
            chunkCntMax[i], bytes_to_compact_number(min),
            bytes_to_compact_suffix(min), bytes_to_compact_number(max),
            bytes_to_compact_suffix(max), chunkMemUsed[i] >> 10);
    }
...
#undef COPY_FIELD
  ...
  void on_alloc(void *ptr, bool aligned = false) {
    ...
    size_t asize = aligned ? dlmalloc_usable_size_aligned(ptr)
    ...
    while (cur_ptr > max_ptr)
      if (interlocked_compare_exchange(max_ptr_in_use, cur_ptr, max_ptr) ==
          max_ptr)
        break;
      else
  ...
  void on_free(void *ptr, bool aligned = false) {
    ...
    ptrdiff_t asize = aligned ? dlmalloc_usable_size_aligned(ptr)
                              : sys_malloc_usable_size(ptr);
...
int64_t dagor_memory_stat::get_malloc_call_count() {
  return interlocked_acquire_load(muc.malloc_cnt) +
         interlocked_acquire_load(muc.free_cnt) +
         interlocked_acquire_load(muc.realloc_cnt);
}
size_t dagor_memory_stat::get_memchunk_count(bool _crt) {
  ...
  return interlocked_acquire_load(muc.ptr_in_use) +
         (_crt ? interlocked_acquire_load(muc_crt.ptr_in_use) : 0);
}
size_t dagor_memory_stat::get_memory_allocated(bool _crt) {
  ...
  if (_crt)
    if (size_t mem_in_use = get_sysmem_in_use())
      return mem_in_use;
#endif
  return interlocked_acquire_load(muc.total_alloc) +
         (_crt ? interlocked_acquire_load(muc_crt.total_alloc) : 0);
}
...
struct MemUsageStats0 {
  ...
  void on_alloc(void *ptr, bool aligned = false) {
    ...
    int asize = int(((aligned ? dlmalloc_usable_size_aligned(ptr)
                              : sys_malloc_usable_size(ptr)) +
                     15) /
                    16);
...
size_t dagor_memory_stat::get_memory_allocated(bool _crt) {
  ...
  if (_crt)
    if (size_t mem_in_use = get_sysmem_in_use())
      return mem_in_use;
#endif
  return 16 * (size_t(interlocked_acquire_load(muc.total_alloc_x16)) +
               (_crt ? interlocked_acquire_load(muc_crt.total_alloc_x16) : 0));
}

#endif

int dagor_memory_stat::get_memory_allocated_kb(bool with_crt) {
  return int(dagor_memory_stat::get_memory_allocated(with_crt) >> 10);
}
...
#if !_STD_RTL_MEMORY ||                                                        \
    (MEM_DEBUGALLOC > 0) // debug allocator requires crit section for changing
...
static void set_memory_thread_safe(bool is_safe) {
  ...
  if (is_safe != _dlmalloc_threadsafe) {
    ...
    // debug("=== switched to %sthead-safe mem management (%d refs)",
...
#if FILL_ALLOCATED_MEMORY
  FILL_MEM_WITH_PATTERN(mem, NEW_MEMORY_FILL_PATTERN,
                        dlmalloc_usable_size_aligned(mem));
#endif
...
inline void *mt_dlrealloc(void *mem, size_t bytes) {
  ...
  if (memDiff > 0)
    FILL_MEM_WITH_PATTERN((char *)new_mem + prevMemSize,
                          NEW_MEMORY_FILL_PATTERN, memDiff);
#endif
...
#if MEASURE_EXPAND_EFF ||                                                      \
    (defined(_STD_RTL_MEMORY) &&                                               \
     !((_TARGET_PC_WIN | _TARGET_XBOX) && (_TARGET_64BIT | USE_MIMALLOC)))
  size_t asz = sys_malloc_usable_size(p);
#endif

  MUC_ON_MEM_FREE(muc, p);
#if defined(_STD_RTL_MEMORY) && (_TARGET_PC_WIN | _TARGET_XBOX) &&             \
    (_TARGET_64BIT | USE_MIMALLOC)
...
#if MEASURE_EXPAND_EFF
  if (asz >= 2048) {
    interlocked_increment(asz <= (16 << 10) ? cnt_expand_total_16K
                                            : cnt_expand_total);
    if (pn)
      interlocked_increment(asz <= (16 << 10) ? cnt_expand_ok_16K
                                              : cnt_expand_ok);
  }
#endif
  return pn != nullptr;
...
CRT_PREFIX void *mt_dlmalloc_crt(size_t bytes) {
  ...
  if (mspace_footprint(_mspace_crt) > dagormem_max_crt_pool_sz &&
      !_mspace_crt_begin_fatal) {
    _mspace_crt_begin_fatal = true;
    debug("Too large footprint for CRT mempool: %llu, last alloc:(%llu)",
          mspace_footprint(_mspace_crt), bytes);
  }
...
CRT_PREFIX void *mt_dlmemalign_crt(size_t bytes, size_t al) {
  ...
  if (mspace_footprint(_mspace_crt) > dagormem_max_crt_pool_sz &&
    ...
    debug("Too large footprint for CRT mempool: %llu, last alloc:(%llu, "
          "align=%d)",
          mspace_footprint(_mspace_crt), bytes, al);
  }
...
CRT_PREFIX void *mt_dlrealloc_crt(void *mem, size_t bytes) {
  ...
  if (mspace_footprint(_mspace_crt) > dagormem_max_crt_pool_sz &&
      !_mspace_crt_begin_fatal) {
    _mspace_crt_begin_fatal = true;
    debug("Too large footprint for CRT mempool: %llu, last realloc(%p, %llu)",
          mspace_footprint(_mspace_crt), mem, bytes);
  }
...
public:
  ...
  void *allocAligned(size_t sz, size_t alignment) override {
    ...
    if (!p) {
      ...
      dumpUsedMem(),
          DAG_FATAL("Not enough memory to alloc %llu bytes (alignment: %u)", sz,
                    alignment);
    }
    memory_tracker.addBlock(this, p, sz);
    return p;
  ...
  void *realloc(void *p, size_t sz) override {
    ...
    size_t asz = sys_malloc_usable_size(p);
    if (asz > 2048 && sz > asz) {
      interlocked_decrement(asz <= (16 << 10) ? cnt_expand_total_16K
                                              : cnt_expand_total);
      interlocked_increment(asz <= (16 << 10) ? cnt_expand_r_total_16K
                                              : cnt_expand_r_total);
      if (mt_expand(p, sz)) {
        interlocked_decrement(asz <= (16 << 10) ? cnt_expand_ok_16K
                                                : cnt_expand_ok);
        interlocked_increment(asz <= (16 << 10) ? cnt_expand_r_ok_16K
                                                : cnt_expand_r_ok);
        memory_tracker.addBlock(this, p, sz);
        return p;
      }
    ...
    if (!np && sz) {
      ...
      dumpUsedMem(),
          DAG_FATAL("Not enough memory in realloc(%p,%llu) call", p, sz);
    }
...
void get_memory_stats(char *buf, int buflen) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    lbw.aprintf("  %scalls(malloc/realloc/free):  %9d/%9d/%9d\n",
                i ? "crt " : "",
                (int)(interlocked_acquire_load(m.malloc_cnt) & 0xFFFFFFFF),
                (int)(interlocked_acquire_load(m.realloc_cnt) & 0xFFFFFFFF),
    ...
    lbw.aprintf("  %smemory(max/now):             %9llu/%9llu  (%6d/%6d K)\n",
                ...
                interlocked_acquire_load(m.total_alloc),
                interlocked_acquire_load(m.max_alloc) >> 10,
  ...
  lbw.aprintf("  dlmalloc allocated(used/total/free):  %6dK/%6dK/%6dK\n",
              ma.uordblks >> 10, dlmalloc_footprint() >> 10, ma.fordblks >> 10);
#endif
...
#else
  lbw.aprintf(
      "%dK in %d ptrs (CRT: %dK in %d)\n",
      size_t(interlocked_acquire_load(::muc.total_alloc_x16)) * 16 / 1024,
      interlocked_acquire_load(::muc.ptr_in_use),
      size_t(interlocked_acquire_load(::muc_crt.total_alloc_x16)) * 16 / 1024,
...
static void
win_enable_low_fragmentation_heap() // https://msdn.microsoft.com/en-US/library/aa366750.aspx
{
#if _TARGET_PC_WIN
  ...
  for (int i = 0; i < numHeaps; ++i)
    HeapSetInformation(heaps[i], HeapCompatibilityInformation, &lfhFlag,
                       sizeof(lfhFlag));
#endif
}

#define DECLARE_MEM_STORAGE(NM_STOR, CLS)                                      \
  alignas(CLS) static char NM_STOR[sizeof(CLS)]
...
static void real_init_memory() {
  ...
  globmem =
      new (glob_mem_storage, _NEW_INPLACE) StdDlmallocAllocator("GlobMem");
  scriptmem =
      new (lua_mem_storage, _NEW_INPLACE) StdDlmallocAllocator("ScriptMem");
  defaultmem = tmpmem;
...
#define FINAL_ALLOC(M)                                                         \
  reinterpret_cast<DagDebugMemAllocator *>(M ? (void *)M                       \
                                             : (void *)std_mem_storage)
...
static void real_init_memory() {
  ...
  sourcemem =
      new (source_mem_storage, _NEW_INPLACE) StdDlmallocAllocator("SourceMem");
  ...
  globmem =
      new (glob_mem_storage, _NEW_INPLACE) DagDebugMemAllocator("GlobMem");
  scriptmem =
      new (lua_mem_storage, _NEW_INPLACE) DagDebugMemAllocator("ScriptMem");
#else
  tmpmem = stdmem;
  inimem = stdmem;
  midmem = stdmem;
...
extern "C" void *memalloc_aligned_default(size_t sz, size_t al) {
  return defaultmem->allocAligned(sz, al);
}
extern "C" void memfree_aligned_default(void *p) { defaultmem->freeAligned(p); }
extern "C" void *memrealloc_default(void *p, size_t sz) {
  return defaultmem->realloc(p, sz);
}
extern "C" int memresizeinplace_default(void *p, size_t sz) {
  return int(defaultmem->resizeInplace(p, sz));
}
...
void *operator new(std::size_t s) DAG_THROW_BAD_ALLOC {
  return FINAL_ALLOC(defaultmem)->alloc(s);
}
void *operator new(std::size_t s, const std::nothrow_t &) DAG_NOEXCEPT {
  return FINAL_ALLOC(defaultmem)->tryAlloc(s);
}
void *operator new[](std::size_t s) DAG_THROW_BAD_ALLOC {
  return FINAL_ALLOC(defaultmem)->alloc(s);
}
...
void *operator new(std::size_t sz, std::align_val_t al) {
  return FINAL_ALLOC(defaultmem)->allocAligned(sz, (size_t)al);
}
void *operator new[](std::size_t sz, std::align_val_t al) {
  return FINAL_ALLOC(defaultmem)->allocAligned(sz, (size_t)al);
}
void operator delete(void *ptr, std::align_val_t) noexcept {
  return FINAL_ALLOC(defaultmem)->freeAligned(ptr);
}
void operator delete[](void *ptr, std::align_val_t) noexcept {
  return FINAL_ALLOC(defaultmem)->freeAligned(ptr);
}

...
template <typename T> using ConstSpan = dag::Span<T const>;
...
inline DAGOR_NOINLINE void
span_index_assert([[maybe_unused]] const char *name, [[maybe_unused]] int idx,
                  [[maybe_unused]] int dcnt, [[maybe_unused]] const void *self,
                  [[maybe_unused]] const void *dptr, [[maybe_unused]] int szt) {
  G_ASSERT_FAIL(
      "Index out of bounds: %s[%d] dcnt=%d this=0x%p dptr=0x%p sizeof(T)=%d",
      name, idx, dcnt, self, dptr, szt);
}
...
template <typename T> class dag::Span {
public:
...
public:
  /// Default constructor. Pointer is initialized to NULL and number of elements
  /// to 0.
  Span() = default;
  ...
  template <typename V, typename U = T,
            bool s = dag::is_same<U, typename V::value_type const>::is_true>
  ...
  constexpr Span<T const> subspan(uint32_t ofs) const {
    return subspan(ofs, size() - ofs);
  }
  constexpr Span<T const> first(uint32_t cnt) const { return subspan(0, cnt); }
  constexpr Span<T const> last(uint32_t cnt) const {
    return subspan(size() - cnt, cnt);
  }
...
template <class T> inline dag::Span<T> make_span(T *p, intptr_t n) {
...
template <class T, size_t N> inline dag::Span<T> make_span(T (&array)[N]) {
...
template <typename V> inline const auto *safe_at(const V &v, uint32_t idx) {
  return idx < v.size() ? v.data() + idx : nullptr;
}
template <typename V> inline auto *safe_at(V &v, uint32_t idx) {
  return idx < v.size() ? v.data() + idx : nullptr;
}
...
template <class T> inline void mem_set_0(dag::Span<T> v) {
  memset(v.data(), 0, data_size(v));
}
template <class T> inline void mem_set_ff(dag::Span<T> v) {
  memset(v.data(), 0xFF, data_size(v));
}
template <class T>
...
template <class V, typename T = typename V::value_type,
          int TSZ = sizeof(*(T) nullptr)>
inline void erase_ptr_items(V &v, uint32_t at, uint32_t n) {
#ifdef _DEBUG_TAB_
  if (n)
    make_span_const(v).operator[](at),
        make_span_const(v).operator[](at + n - 1);
#endif

...
size_t mmap_allocated_bytes = 0;
size_t mmap_allocated_chunks = 0;

/* Win32 MMAP via VirtualAlloc */
static FORCEINLINE void* win32mmap(size_t size) {
  unsigned int flags = 0;

  void* ptr = VirtualAlloc(0, size, MEM_RESERVE|MEM_COMMIT|MEM_TOP_DOWN|flags, PAGE_READWRITE);
  if (ptr)
  {
    mmap_allocated_bytes += size;
    mmap_allocated_chunks++;
  }
...
#define chunkfd(p)          ((p)->head & FLAG4_BIT)
#define clear_chunkfd(p)    ((p)->head &= ~FLAG4_BIT)
#define set_chunkfd(p)      ((p)->head |= FLAG4_BIT)
...
#elif defined(_MSC_VER) && _MSC_VER>=1300 && _TARGET_PC && DAGOR_DBGLEVEL<2
#define compute_tree_index(S, I)\
{\
  size_t X = S >> TREEBIN_SHIFT;\
  if (X == 0)\
...
static void* internal_realloc(mstate m, void* oldmem, size_t bytes, int force_inplace) {
...
#endif /* FOOTERS */
    return internal_realloc(m, oldmem, bytes, 0);
  }
}

void* dlresize_inplace(void* oldmem, size_t bytes)
{
  if (!oldmem || !bytes)
    return bytes ? NULL : oldmem;
#if ! FOOTERS
  mstate m = gm;
#else /* FOOTERS */
  mstate m = get_mstate_for(mem2chunk(oldmem));
...
void* mspace_realloc(mspace msp, void* oldmem, size_t bytes) {
  ...
  if (bytes == 0) {
    ...
    return internal_realloc(ms, oldmem, bytes, 0);
  }
}
...
#if !ONLY_MSPACES
const struct malloc_segment *dlmainseg() { return &_gm_.seg; }
#endif
